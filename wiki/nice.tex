...
constituents. This allows the reader to read the expression, and delve into the
details, if the reader wishes to do so. As long as important deails are
retained, this is a useful technique for making your programs more readable.
...
well.}, more suitable for writing \emph{procedures} like this. In particular,
...
in the sense that a name defined further up can be used in definitions further
down. If we represent ``state'' as the set of names available for use in an
expression we can specify how the state develops throughout the
\mono{let}-block: 
...
Let us concentrate around a single statement:

\begin{code}
dirtyWhites, dirtyColours = split(dirtyLaundry)
\end{code}

If we let \var{dirtyLaundry} be a basket of mixed dirty laundry, we'de like for
the effect of this \emph{assignment} to be that \var{dirtyWhites} becomes a
pile of dirty whites, and \var{dirtyColours} becomes a pile of dirty colours,
while \var{dirtyLaundry} becomes an \emph{empty} basket. That is, we'd like the
laundry is \emph{moved} from the basket to the respective piles.
