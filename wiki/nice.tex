...
We refer to lines 1--7 as the ``\mono{let}-block'', lines 2--4 as the
``\mono{let}-definitions'', and line 7 as the ``\mono{let}-expression''.

In functional programming, the \mono{let}-block can be used to reduce the
...
It seems overly excessive to have the state at the \mono{let}-expression
contain all these ``intermediate'' names, such as \var{dirtyWhites},
\var{cleanWetColours}, etc. All we really need in the \mono{let}-expression is
...
While we could attempt mitigate for this using nested \mono{let}-blocks or
