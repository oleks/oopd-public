...
the details, if necessary. As long as important details are retained in the
\mono{let}-expression, this is a useful technique for making your programs more
readable.
...
functions easier to comprehend.}, the code will not be as easy as 1-2-3, i.e.
as simple as our initial specification of the procedure.
...
\emph{concrete values} these names actually refer to. Indeed, before the above
statement, \var{dirtyLaundry} is a basket of dirty clothes; after it, it is an
empty basket.

\begin{definition}

A mutable value, also known as a variable, is a named entity that can refer to
different concrete values throughout the lifetime of a program.

\end{definition}

If we represent baskets and piles as good old lists, we'd like
...
Let us hence consider the \var{split} procedure. 
